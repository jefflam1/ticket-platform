...
const EventCard = ({ eventId }: { eventId: Id<'events'> }) => {
  ...
  return (
    ...
    >
      ...
      {/* event details */}
      <div className={`p-6 bg-red-300 ${imageUrl ? 'relative' : ''}`}>
        <div className='flex items-start justify-between'>
          {/* event name and owener badge */}
          <div>
            <div className='flex flex-col items-start gap-2'>
              {isEventOwner && (
                <span className='inline-flex items-center gap-1 bg-blue-500/90 text-white px-2 py-1 rounded-full text-xs font-medium'>
            ...
            {isPastEvent && (
              <span className='inline-flex items-center px-2.5 py-0.5  bg-gray-100/90 text-gray-800 mt-2 rounded-full text-xs font-medium'>
                Past event
              </span>
            )}
          </div>
      ...
      {/* event detail */}
      <div className='mt-4 gap-3 bg-amber-300'>
